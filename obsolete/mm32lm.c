...
int main (void)
{
  int gotOne, pin ;
  int myCounter [8] ;
  int tidx[8]={17783, 17784, 0, 0, 0, 0, 0, 0};
  char payload[500];

  mosquitto_lib_init();
  struct mosquitto *mosq = mosquitto_new("capteur1", true, NULL);
  mosquitto_connect(mosq, "192.168.1.39", 1883, 60);
...
//	  sprintf(payload, "{ \"idx\" : 17784, \"nvalue\" : 0, \"svalue\" : \"%d\" }\n",  1);
//	  sprintf(payload, "{ \"idx\" : 17783, \"nvalue\" : 0, \"svalue\" : \"%d\" }\n",  globalCounter [pin]);
//	  sprintf(payload, "{ \"idx\" : 17784, \"nvalue\" : 0, \"svalue\" : \"%d\" }\n",  globalCounter [pin]);
	  printf(payload);
